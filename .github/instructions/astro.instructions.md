...
- Document every reusable component's `Props` interface as its public API. Add a short TSDoc comment to the interface and document props whose purpose or allowed values are not obvious from their names and types.
- Type component imports and helper return values
- Prefer `interface` for object-shaped contracts and `import type` for type-only imports.
...
- Comments should explain intent, accessibility reasoning, or non-obvious rendering decisions. Do not restate the markup or expression they sit beside, and remove stale comments when the component changes.
